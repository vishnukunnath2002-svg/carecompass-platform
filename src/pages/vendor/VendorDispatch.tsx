...
export default function VendorDispatch() {
  ...
  useEffect(() => {
    supabase.from('orders').select('*').in('status', ['confirmed', 'shipped']).order('created_at', { ascending: false }).then(({ data }) => {
      ...
      <Card>
        <CardContent className="p-0">
          {orders.length === 0 ? (
            ...
            <Table>
              <TableHeader><TableRow><TableHead>Order #</TableHead><TableHead>Status</TableHead><TableHead>Tracking</TableHead><TableHead>Total</TableHead><TableHead>Date</TableHead></TableRow></TableHeader>
              <TableBody>
                {orders.map((o) => (
                  <TableRow key={o.id}>
                    <TableCell className="font-mono">{o.order_number}</TableCell>
                    <TableCell><Badge variant="outline">{o.status}</Badge></TableCell>
                    <TableCell className="font-mono text-sm">{o.tracking_number || '—'}</TableCell>
