...
export default function ShopProducts() {
  ...
  const handleAddToCart = (product: Product) => {
    toast({
      title: 'Added to cart',
      description: `${product.name} added. Cart feature coming soon.`,
    });
  };
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <Card key={i} className="animate-pulse"><CardContent className="h-56 p-6" /></Card>
          ))}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <Card key={p.id} className="border shadow-card transition-all hover:shadow-elevated overflow-hidden">
              ...
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  ...
                  {p.is_prescription_required && (
                    <Badge variant="outline" className="ml-2 shrink-0 text-xs border-destructive text-destructive">Rx</Badge>
                  )}
                </div>
