...
export default function AgencyReviews() {
  ...
  useEffect(() => {
    supabase.from('reviews').select('*').eq('target_type', 'agency').order('created_at', { ascending: false }).then(({ data }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Customer reviews for your agency and staff.</p>
      </div>
      {reviews.length === 0 ? (
